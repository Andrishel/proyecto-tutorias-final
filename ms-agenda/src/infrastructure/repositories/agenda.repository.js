...
try {
 ...
 return res.rows[0]; // Retorna el bloqueo recién creado con su ID
        ...
        // --- MISION 1: MANEJO DE ERROR 409 ---
        if (err.code === '23505') { // Código paar el error de Postgres para Unique Violation
            const errorConflict = new Error('Conflicto de tiempo: El horario ya fue reservado.');
            errorConflict.statusCode = 409; // El conflicto
            throw errorConflict;
        }
        // -------------------------------------
